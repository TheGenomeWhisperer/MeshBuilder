...
public class Mesh
{
    ...
    private static Vector3 GetClosestNode(Vector3[] hotspots, Vector3 destination)
    {
    ...
    private static Vector3[] InsertionSortHotspots(Vector3[] hotspots)
    {
        Vector3 temp;
    ...
    private static Vector3[] InsertionSortHotspots(Vector3[] hotspots, Vector3 destination)
    {
        Vector3 temp;
        for (int i = 0; i < hotspots.Length - 1; i++)
        {
    ...
    {   
        ...
        if (API.Me.Distance2DTo(destination) > 3f)
        {
            ...
            API.Print(API.Me.Distance2DTo(route[0]));
            ...
            if (API.Me.Distance2DTo(route[0]) >= 15f)
            {
                ...
                if (IsAnyMeshClose(15f))
                {
                    ...
                    {
                        while(!API.CTM(route2[i]) && API.Me.Distance2DTo(route2[i]) > 1.5f)
                        {
                            yield return 100;
                        }
                    }
                ...
                while(!API.MoveTo(route[route.Length - 1]) && API.Me.Distance2DTo(route[route.Length - 1]) > 1.5f)
                {
                    yield return 250;
                }
            }
            ...
            {
                while(!API.CTM(result[i]) && API.Me.Distance2DTo(result[i]) > 1.5f)
                {
                    yield return 100;
                }
            }
    ...
    private static Vector3[] CreatePath(Vector3[] hotspots, Vector3 destination)
    {
        ...
        for (int i = 1; i < hotspotList.Count - 1; i++)
        {
        ...
        while(NotDoneSorting)
        {
            ...
            for (int i = count + 1; i < hotspotList.Count - 1; i++)
            {
